...
Coder - Bruno Patrocinio - Fatec.  V. 4.2.0  - com display OLED 0.96 - 128x64" 
*/
...
void setup() {
  ...
  //inicializa os pinos digitais(led e buzzer)
...
void loop() {
 ...
 printaSerial(); // imprime no monitor serial do arduino, apenas.
 verificaHorario(); //função p/ verificar horario de parada e Qtd de carros restantes
 printDisplay(); //imprime no display
...
void verificaHorario(){
  ...
  //para a execução do laço for e ifs internos
    ...
    if(agora.dayOfTheWeek() != 6 || agora.dayOfTheWeek() != 7){ // se não for sabado e nem domingo, faça.
        ...
        if(tempo[0] == 8){
          if(tempo[1] >= 25 && tempo[1] < 35){
            ...
            carrosRestantes = (int)resultado / tempoLinha;
             ...
             if(millis() - timer2 > 120000){ // a cada 2 min. apita por 1,5 seg. e para
              tone(pinoSom, 1500, 800);
